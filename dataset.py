    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'Train':
            ...
            sample = {'sketch_img': sketch_img,
                       ...
                       'mixup_label' : self.name2num[sketch_path_mixup.split('/')[0]], 
                       'lam' : lam}
            ...
            sample = {'sketch_img': sketch_img,
                     'sketch_label': self.name2num[sketch_path_mixup.split('/')[0]]}
...
def collate_self_train(batch):
    ...
    for i_batch in batch:
        ...
        batch_mod['mixup_label'].append(i_batch['mixup_label'])        
        batch_mod['lam'].append(i_batch['lam'])


    batch_mod['sketch_img'] = torch.stack(batch_mod['sketch_img'], dim=0)
    batch_mod['sketch_label'] = torch.tensor(batch_mod['sketch_label'])
    batch_mod['mixup_label'] = torch.tensor(batch_mod['mixup_label'])
    batch_mod['lam'] = torch.tensor(batch_mod['lam'])
